...
from ..data.image_processor import get_preprocessor
...
async def startup_event():
    ...
    # Initialize index
    # Note: In production, you would load pre-built index here
    index = get_index(
        index_type="faiss",
        dim=512,
        metric="cosine"
    ...
    *,
    ...
    try:
        ...
        return SearchResponse(
            results=results,
            query_time=0.0  # TODO: Add actual timing
        )

    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error searching similar images: {str(e)}"
        )
...
async def add_to_index(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error adding image to index: {str(e)}"
        )